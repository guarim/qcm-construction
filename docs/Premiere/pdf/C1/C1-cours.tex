	...
	\begin{block}{CLI : command \texttt{chmod}}
		\begin{itemize}
			...
			\item<7-> La commande \texttt{chmod} permet de modifier les droits sur un fichier dont on est propriétaire. En voici quelques exemples :
			      \begin{itemize}
				      \item<8->{\texttt{chmod g+w monfichier} :} \onslide<9->{\footnotesize Ajoute (\texttt{+}) au groupe (\texttt{g}) le droit d'écriture (\texttt{w})}
				      \item<10->{\texttt{chmod u+x monfichier} :} \onslide<11->{\footnotesize Ajoute (\texttt{+}) au propriétaire (\texttt{u}) le droit d'éxécution (\texttt{x})}
				      \item<12->{\texttt{chmod og-r monfichier} :} \onslide<13->{\footnotesize Enlève (\texttt{-}) au groupe et aux autres (\texttt{og}) le droit de lecture (\texttt{r})}
				      \item<14->{\texttt{chmod a-r monfichier} :} \onslide<15->{\footnotesize Enlève (\texttt{-}) à tout le monde (\texttt{a}) le droit de lecture (\texttt{r})}
			      \end{itemize}
		\end{itemize}
	\end{block}
\end{frame}
