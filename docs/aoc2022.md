...
map[exit[0]]=map[exit[0]].replace('E','z')
...
### Partie A
```python
reader = open("inputday13.txt")
pairs = reader.read().strip().split('\n\n')

def get_lr(pair):
    l,r = pair.split('\n')
    left, right = eval(l), eval(r)
    return left, right
...
print(sum(i+1 for i in range(len(pairs)) if compare(*get_lr(pairs[i]))))
```
...
for p in pairs:
    ...
    packet1 = eval(pac[0])
    packet2 = eval(pac[1])
    packets = packets + [packet1] + [packet2]
...
def get_pos(packet,packet_list):
    return sum(compare(p,packet) for p in packet_list if p!=packet)+1
    
print(get_pos(divider1,packets)*get_pos(divider2,packets))
```
...
def move_sand(l,c):
    if l+1>=height:
        return True,True,l,c
    if cave[l+1][c] == '.':
        cave[l+1][c] = 'O'
        cave[l][c] = '.'
        return False,False,l+1,c
    if c-1<0:
        return True,True,l,c
    if cave[l+1][c-1] == '.':
        cave[l+1][c-1] = 'O'
        cave[l][c] = '.'
        return False,False,l+1,c-1
    if c+1>=width:
        return True,True,l,c
    if cave[l+1][c+1] == '.':
        cave[l+1][c+1] = 'O'
        cave[l][c] = '.'
        return False,False,l+1,c+1
    return False,True,l,c


exit,count = False,0
while not exit:
    count += 1
    lig,col,stuck = 0,500,False
    while not stuck:
        exit,stuck,lig,col = move_sand(lig,col)
...
for line in data:
    ...
    for i in range(1,len(points)):
        ...
        ystart, xstart = eval(start)
        yend, xend = eval(end) 
        if max(xstart,xend)>high :
            high = max(xstart,xend)
        if xstart != xend:
            for x in range(min(xstart,xend),max(xstart,xend)+1):
                cave[x][ystart] = '#'
        else:
